...
TODO: assert -- ПРОВЕРИТЬ ВСЕ АССЕРТЫ, ЧТОБЫ В РЕЛИЗЕ НЕ ВЫПАДАЛ КОД КОТОРЫЙ ДОЛЖЕН ВЫПОЛНИТЬСЯ.
...
+3) GenerateMap чтоб наконец запоминал бы путь.
...
+10) "Отмена" на диалоге Generate map заработала, и вообще чтоб при эксепшене в корень нормлаьно выходила.
11) Выход менюху хорошо бы.
...
3) редактирование
4) Выход из редактора в кореневое меню.
...
см map_editor_impl.h


+MainMenu LoadMap если скормили корявый файл не падать,
+а так же проверить что повторная загрузка уже правильного файла
 приводит к некорректной работе.


+TODO: Проверить загрузку и нициализацию ресурсов, хранение в одном месте.
Безпорядок там.
:::::: сейчас в GameState всё.
